...
public class IconFactory {
    ...
    public IconFactory(double x, double y, int width, int height) {
        //this.ts = ts;
        this.x = x;
        this.y = y;
        envWidth = width;
    ...
    // used instead of getRememberMeIconNode.
     public Node getRememberMeIcon(double x1x2, double y1y2){
         Vector3f v = new Vector3f((float)(x1x2/20-envWidth/20), 0.f, (float)((y1y2/20)-envHeight/20));
         x = v.getX();
         y = v.getZ();
         material = new Material3D(Color.white, 0.0, 0.0);
         //this.ms = ms;
