...
public class ThingCreator {
        ...
        try {
            switch (category) {
                ...
                case Constants.categoryBRICK:
                    //e.thingTsPool.put(t, DisplaySystem.getDisplaySystem().getRenderer().createTextureState());
                    th = new Brick(x, y, e);
                    th.setMaterial(new Material3D(1.0, Color.magenta));//only jewel shines
                    break;

                case Constants.categoryJEWEL:
                    //e.thingTsPool.put(t, DisplaySystem.getDisplaySystem().getRenderer().createTextureState());
                    th = new Jewel(x, y, e);
                    th.setMaterial(new Material3D(1.0, Color.green));//only jewel shines
                    break;

                case Constants.categoryNPFOOD:
                    //e.thingTsPool.put(t, DisplaySystem.getDisplaySystem().getRenderer().createTextureState());
                    th = new NonPerishableFood(x, y, e);
                    th.setMaterial(new Material3D(Color.green, 1.0, Constants.NPFOOD_ENERGY, 0.0)); //non-perishable food has energy = 2.0!
                ...
                case Constants.categoryPFOOD: // one apple => more calories than one nut. But it may be rotten.
                    //e.thingTsPool.put(t, DisplaySystem.getDisplaySystem().getRenderer().createTextureState());
                    //10 minutes food
                    th = new PerishableFood(Constants.VALID_PERIOD_SECS, x, y, e);
                    th.setMaterial(new Material3D(Color.red, 1.0, Constants.PFOOD_ENERGY, 0.0)); //perishable food has energy = 1.0!
                ...
                case Constants.categoryCAGE:
                    //e.thingTsPool.put(t, DisplaySystem.getDisplaySystem().getRenderer().createTextureState());
                    th = new Cage(x, y, e);
                    th.setMaterial(new Material3D(Color.yellow, 1.0, 0, 0));
                    break;
                case Constants.categoryDeliverySPOT:
                    //e.thingTsPool.put(t, DisplaySystem.getDisplaySystem().getRenderer().createTextureState());
                    //reuse texture pool:
                    th = new DeliverySpot(x, y, e);
                    break;
            }
    ...
    public synchronized Thing createBrick(int color, double x1, double y1, double x2, double y2) {
        ...
        //e.oMsPool.put(t, DisplaySystem.getDisplaySystem().getRenderer().createMaterialState());
        //e.thingTsPool.put(t, DisplaySystem.getDisplaySystem().getRenderer().createTextureState());
        Thing th = new Brick(x1, y1, x2, y2, e);
    ...
    public synchronized Thing reCreateThing(int category, int subcategory, String color, double x, double y, double x2, double y2, boolean wasHidden) {
        ...
        switch (category) {
            ...
            case Constants.categoryBRICK:
                ...
                th = createThing(Constants.categoryBRICK, x, y);
                th.setX2(x2);
                th.setY2(y2);
                th.getMaterial().setColor(Constants.translateIntoColor(color));
                        //th.addBrick(app);
                ...
                th.state = 1;
                System.out.format("Created %s Brick from %.2f,%.2f -> %.2f,%.2f\n",color,x,y,x2,y2);
                break;

            case Constants.categoryJEWEL:
                //th = new Jewel(x, y, e);
                //th.getMaterial().setColor(Constants.translateIntoColor(color));
                th = createThing(Constants.categoryJEWEL, x,y);
                th.setMaterial(new Material3D(Constants.translateIntoColor(color)));
                th.state = 1;
                System.out.format("Created %s Jewel at %.2f,%.2f\n",color,x,y);
                break;
            ...
            case Constants.categoryNPFOOD:
                ...
                //e.thingTsPool.put(t, DisplaySystem.getDisplaySystem().getRenderer().createTextureState());
//                th = new NonPerishableFood(x, y, e);
//                th.setMaterial(new Material3D(Color.orange, 1.0, Constants.NPFOOD_ENERGY, 0.0)); //non-perishable food has energy = 2.0!
                th = createThing(Constants.categoryNPFOOD, x,y);
                th.state = 1;
                System.out.format("Created Nut at %.2f,%.2f\n",x,y);
                break;
            case Constants.categoryPFOOD:
                ...
                //th = new PerishableFood(Constants.VALID_PERIOD_SECS, x, y, e);
                //th.setMaterial(new Material3D(Color.red, 1.0, Constants.PFOOD_ENERGY, 0.0)); //perishable food has energy = 1.0!
                th = createThing(Constants.categoryPFOOD, x,y);
                System.out.format("Created Apple at %.2f,%.2f\n",x,y);
                th.state = 1;
                break;
